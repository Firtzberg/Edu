...
{{ link_to_route('Kategorija.edit', 'Preimenuj', array($kategorija->id), array('class' => 'btn btn-default')) }}
@endif
<h3>Podkategorje <small>ukupno {{ $kategorija->podkategorije->count() }}</small></h3>
<div id="list-podkategorije" class="container">
    @if($kategorija->podkategorije->count() < 1)
    <p>Nema podkategorija.</p>
    @else
    <table id="podkategorije-table" class="table table-striped"><tbody>
            <tr>
...
<h3>Predmeti <small>ukupno {{ $kategorija->predmeti->count() }}</small></h3>
